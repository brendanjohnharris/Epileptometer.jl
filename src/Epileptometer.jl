...
function calculate(file::File; dt = 1 / file.metadata["SamplingFrequency"],
                   windowlen = 1000) # * Assume it is ok to concatenate data from either side of the seizure event
    x, e = formateeg(file)
    Off = x[𝑡 = .!e]
    On = x[𝑡 = e]
    ...
    x = map([Off, On], [0, 1]) do O, e # * Buffer to smaller time series
        ...
        O = set(O, 𝑡 => ts)
        O = buffer.(eachslice(O, dims = 2), windowlen, 0) # 1000 samples, 0 overlap
...
function calculate(session::Session; kwargs...)
    files = session |> BIDSTools.files
    Fs = calculate.(files)
    obs = vcat(lookup.(Fs, Obs)...)
    cat(Fs..., dims = Obs(obs))
end
function calculate(subject::Subject; kwargs...)
